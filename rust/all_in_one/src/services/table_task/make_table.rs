            ...
            match data {
                Ok(data) => {
                    ...
                    for content in cells{
                            rows.push(Row::new(content , if count==0 {RowType::HeaderRow} else {RowType::DataRows} , if count==0 {data.headerRow.fontSize} else {data.dataRows.fontSize} , data.pageWidth , cell_count));
                            count += 1;
                    }
